...
(function () {
  ...
  links.forEach(function(a){
    /* a link may observe a different element than it scrolls to (data-spy):
       e.g. "Results" scrolls to #results but lights up from #res-intro, so the
       intro lights "Results" alone and the families light "Quantitative" too. */
  ...
  function activate(a){
    ...
    /* if it's a sub-item, also light its parent (e.g. Quantitative -> Results) */
    var sub = a.closest(".toc-sub");
    if (sub) {
      var parentLink = sub.parentElement.querySelector(":scope > a");
  ...
  var zoomImgs = document.querySelectorAll('.paper-fig img, .method-img, .res-img, .res-table-img, .abl-img, .qual-img, .teaser-img, .walk-img, .meme-img');
  var lb = null;
  ...
  var sf = document.getElementById("share-fb");
  ...
  if (sf) sf.href = "https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent(PAGE_URL);
  ...
  if (toTop) {
    var onScroll = function(){
      toTop.classList.toggle("show", window.pageYOffset > 480);
    };
